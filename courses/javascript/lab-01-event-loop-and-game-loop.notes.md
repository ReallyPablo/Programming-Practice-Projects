...
У грі це видно одразу: симуляція має йти фіксованим кроком (~60 разів/с), а малювання — з частотою монітора. Тому в HUD два числа, не одне.
...
Це дослід M4-1 з лаби, тільки в консолі. У грі той самий `while` у `render` дає ривок раз на секунду.

---

## 3. `requestAnimationFrame`, не `setInterval(16)`

rAF: «виклич мене перед наступним **paint**». Збігається з герцами екрана, на фоновій вкладці браузер майже зупиняє. `setInterval(fn, 16)` пливе, 16 ≠ 16.667, у фоні поводиться інакше.
...
Після M1–M3 ці ідеї лежать так:

| Ідея | Де дивитись |
|---|---|
| rAF + accumulator + clamp | `src/loop.js` |
| HUD: steps/s vs frames/s | `src/render/draw.js` → `drawHud` |
| input як замикання | `src/input.js` |
| фізика без DOM | `src/sim/ship.js` |
| wrap + не lerp крізь арену | `src/sim/arena.js` |
| `lerpAngle`, сітка, DPR | `src/render/draw.js`, `canvas.js` |

Перевірка M1 без читання коду: `steps/s` ≈ 60 на будь-якому моніторі; `frames/s` ≈ герці.

Досліди M4 (busy-wait, `setInterval`, змінний `dt`) роби **тимчасово**, цифри — в README, код знову чистий. Лаба так і просить.
...
Три запуски: дослід §1, §2, §3. Потім відкрий HUD гри. Решту — з лаби, секція Theory.
