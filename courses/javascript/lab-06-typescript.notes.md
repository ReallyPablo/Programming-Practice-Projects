...
Досліди — у файлі `.ts` + `tsc --strict` (або Vite). Наводь курсор на типи. На TS-вакансіях питають не синтаксис, а `unknown`, union і «типів на дроті немає».

---

## Ідея

TS стирається до JS. На дроті типів немає. Структурна типізація: важлива форма, не ім’я. `any` вимикає перевірку; `unknown` змушує звузити. Повідомлення клієнт ↔ сервер — **discriminated union** + `never` у `default`, щоб нова літера ламала збірку, а не прод.
...
На свіжому Node `enum` у «strip types» часто **не** підтримується. Замість нього:
