...
Консоль Chrome. Це друга половина event loop з Lab 1: реакції промісів — **мікрозадачі** (черга, яку браузер спустошує перед наступним таймером). На співбесіді майже завжди просять порядок логів з `then` / `await` / `setTimeout`.
