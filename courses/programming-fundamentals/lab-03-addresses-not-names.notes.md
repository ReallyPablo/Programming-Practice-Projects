...
Лаба — **що здати** (`LOAD`/`STORE`, little-endian, ASan, тег). Фрагмент → `scratch.cpp`, збірка як у [Notes 01](lab-01-a-box-of-bytes.notes.md). ASan сам надрукує репорт у термінал. **Етапи, терміни й питання на захист — у [лабі](lab-03-addresses-not-names.md); тут вони не повторюються.** Цей файл — фрагменти, які ви запускаєте.
...
У ember клітинка — `Byte`, тому індекс `addr+1` це наступний байт. Саме тому `INCH`
рухає `H` на **один** байт, а `p + 1` на `int*` — на чотири. Арифметика вказівників
рахує **елементи**. Не ходи `int*` по пам’яті ember «по одному» — перескочиш клітинки.
...
**Очікуй:** `AB` (якщо за `0x800` лежать `65 66`), і `H` у `regs` зросло на 1.
