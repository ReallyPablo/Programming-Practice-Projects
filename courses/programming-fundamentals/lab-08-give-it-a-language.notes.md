...
Лаба — **що здати** (лексер, asm, три програми, `v1.0.0`). Цей файл — **з чим розібратись**. Лексер і програми — `./build/ember lex …` та `./build/ember programs/fib.asm`. Дрібні C++-досліди — `scratch.cpp` як у [Notes 01](lab-01-a-box-of-bytes.notes.md). **Етапи, терміни й питання на захист — у [лабі](lab-08-give-it-a-language.md); тут вони не повторюються.** Цей файл — фрагменти, які ви запускаєте.
