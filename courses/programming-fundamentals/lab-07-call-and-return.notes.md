...
і **точний** порядок байтів у `CALL`/`RET` — [ISA.md §6](ISA.md#6-how-call-and-ret-work-exactly).
Конвенція виклику (аргумент в `A`, результат в `A`, `B` і `H` рятує викликач) —
[там само](ISA.md#calling-convention).
...
`fact` усередині ember: у README перекажи **конвенцію** з [ISA.md](ISA.md#calling-convention)
...
| `CALL`/`RET` | push **наступного** PC (`PC + 3`), порядок байтів з ISA.md |
| `fact` через `set` | до Lab 8 ще hex, але вже виклики |
| множення | своя підпрограма або свій опкод у `0x7_` — задокументуй |
