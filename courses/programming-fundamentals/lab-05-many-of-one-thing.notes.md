...
**Stride** — скільки байтів пропустити, щоб спуститись на наступний рядок (тут 8). **C-string** — байти до нуля. **Decay** — ім’я масиву в більшості виразів стає вказівником на перший елемент; розмір губиться.

Екран **не** окремий масив на хості. Це 256 байтів пам’яті ember за адресою
`0xA00` ([ISA.md §7](ISA.md#7-the-display-is-memory)). Після малювання зроби
...
- Чому `y * 8 + x / 8`, не навпаки?
- Чому `7 - (x % 8)`? Намалюй байт `0x80` як вісім пікселів.
- Чому cap на `OUTS`?
...
§2, §3, рамка на `show`, і піксель через `STORE [H], A`. Sort — з лаби M5.
