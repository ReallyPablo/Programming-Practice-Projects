...
| **§4** | тінь `x` + `static` | membrane — просто shadowing |
...
C++: `if`, `while`, `for`. CPU: **записати нову адресу в `PC`**. Усе інше — вкладені форми.
...
**Очікуй:** `10 1`, потім зовнішнє `1`. У циклі `a` росте `1 2 3`, `b` щоразу `1`. `static` живе до кінця програми; **ім’я** `c` з внутрішнього блоку назовні не видно. Це весь «мембранний ефект».
...
У C++ це `for` + `if` + `return`. У ember — `LOAD`, `CMP`, `JZ found`, збільшити адресу, `JMP loop`. Одна форма.
...
| ліміт кроків | `run` |
| `find lo hi byte` | хост-цикл |
| countdown | гостьова програма в README |
...
- Переклади `while (a>0) a--;` у тести й стрибки.
