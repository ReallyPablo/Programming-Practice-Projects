...
Опкоди стрибків — група `0x3_` у [ISA.md](ISA.md). Дві пастки звідти: узятий `JMP`
...
| countdown | [ISA.md §9](ISA.md#9-two-programs-to-check-yourself-against) — байти вже пораховані |

Трейс countdown: кожен рядок `PC A Z`. Поруч — `while (a > 0)` на C++. Той самий малюнок.

Свої лістинги пиши з колонкою адрес, як в ISA.md. У Lab 8 асемблер має видати
рівно ці байти — буде з чим звіряти.
...
- Чому взятий `JMP` не додає ще й розмір інструкції?
- Навіщо `CMP`, якщо `SUB` і так виставляє прапорці?
