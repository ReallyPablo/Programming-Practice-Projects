...
Лаба — **що здати** (`JMP`/`JZ`, countdown, `find`, тег). Фрагмент → `scratch.cpp`, збірка як у [Notes 01](lab-01-a-box-of-bytes.notes.md). Countdown і `find` — команди в `./build/ember`. **Етапи, терміни й питання на захист — у [лабі](lab-04-the-shape-of-control.md); тут вони не повторюються.** Цей файл — фрагменти, які ви запускаєте.
...
Опкоди стрибків — група `0x3_` у [ISA.uk.md](ISA.uk.md). Дві пастки звідти: узятий `JMP`
...
| countdown | [ISA.uk.md §9](ISA.uk.md#9-дві-програми-щоб-перевірити-себе) — байти вже пораховані |
