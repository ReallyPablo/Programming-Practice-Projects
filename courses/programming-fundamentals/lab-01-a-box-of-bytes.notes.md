...
Лаба — **що здати** (CMake, dump, peek/poke, тег). Цей файл — **з чим розібратись**: коротка теорія і досліди. Усе — **термінал**, не кнопка Run і не вікно Watch.

**`ember`** — назва вашої програми-комп’ютера (можна перейменувати). **Dump / get / set** — команди в її prompt: надрукувати пам’ять, прочитати байт, записати байт.
...
Перед **кожним** дослідом скажи вголос, що очікуєш. Звіряй з виводом `./scratch`. Проєкт — **після §4**:
...
**`ember`** — ваша C++-програма, яка вдає маленький комп’ютер (4096 байтів + prompt). **Байт** — 8 біт, одна клітинка тієї пам’яті. **Тип** — скільки байтів і які операції; пам’ять про тип не знає. **Компілятор** — перекладач з C++ на машинний код. **UB** (undefined behaviour) — мова не обіцяє, що буде. **Санітайзер** — перевірки, вшиті в бінарник. **ASan** (AddressSanitizer, `-fsanitize=address`) ловить вихід за масив і use-after-free. **UBSan** (`-fsanitize=undefined`) ловить, зокрема, overflow знакового `int`. **Літерал** — значення в коді (`65`, `'A'`). **`const`** — ім’я, через яке ці біти не змінять. Повний словник — у [README.uk.md](README.uk.md#слова-якими-говорять-лаби).
